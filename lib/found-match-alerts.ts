...
import type { CreatedMatchRow } from './matching'
import {
...
export type MatchAlertRow = CreatedMatchRow | {
  _id?: ObjectId
...
/**
 * In-app notifications + SMTP emails when a found document matches lost report(s).
 */
...
export async function notifyMatchesForFoundUpload(matches: MatchAlertRow[]) {
  ...
  if (exactMatches.length > 0) {
    for (const match of exactMatches) {
      const lostReport = (await lostCollection.findOne({
        _id: new ObjectId(match.lostReportId),
      })) as LostReportDoc | null
      const foundReport = (await foundCollection.findOne({
  ...
  const firstLost = (await lostCollection.findOne({
    _id: new ObjectId(first.lostReportId),
  })) as LostReportDoc | null
  const firstFound = (await foundCollection.findOne({
...
/**
 * Email lost owners when a new lost report matches existing found document(s).
 */
