import { NextRequest, NextResponse } from 'next/server'
import { z } from 'zod'
import { ObjectId } from 'mongodb'
import { collections } from '@/lib/mongodb'
import { getStationForUserId } from '@/lib/station-info'
import { sendClaimConfirmationEmail } from '@/lib/claim-email'
import { writeAuditLog } from '@/lib/audit'

const claimSchema = z.object({
  foundReportId: z.string().min(1),
  claimantName: z.string().min(2),
  claimantEmail: z.string().email(),
  claimantPhone: z.string().optional(),
  lostDate: z.string().optional(),
  description: z.string().optional(),
  documentNumber: z.string().optional(),
})

export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const data = claimSchema.parse(body)

    const foundCollection = await collections.foundReports()
    const foundReport = await foundCollection.findOne({
      _id: new ObjectId(data.foundReportId),
    })

    if (!foundReport) {
      return NextResponse.json({ error: 'Document not found' }, { status: 404 })
    }

    if (foundReport.status === 'HANDED_OVER') {
      return NextResponse.json(
        { error: 'This document has already been collected' },
        { status: 400 }
      )
    }

    if (foundReport.status === 'CLAIM_PENDING') {
      return NextResponse.json(
        { error: 'A claim is already pending for this document' },
        { status: 400 }
      )
    }

    const claimsCollection = await collections.claims()
    const claimDoc = {
      foundReportId: foundReport._id,
      claimantName: data.claimantName,
      claimantEmail: data.claimantEmail,
      claimantPhone: data.claimantPhone || null,
      lostDate: data.lostDate ? new Date(data.lostDate) : null,
      description: data.description || null,
      documentNumber: data.documentNumber || null,
      status: 'PENDING',
      createdAt: new Date(),
      updatedAt: new Date(),
    }

    const claimResult = await claimsCollection.insertOne(claimDoc)

    await foundCollection.updateOne(
      { _id: foundReport._id },
      {
        $set: {
          status: 'CLAIM_PENDING',
          updatedAt: new Date(),
        },
      }
    )

    const station = foundReport.userId
      ? await getStationForUserId(foundReport.userId)
      : {
          name: foundReport.foundLocation || 'Collection station',
          address: null,
          phone: null,
          email: null,
        }

    const documentTypeLabel =
      String(foundReport.documentType || 'Document').replace(/_/g, ' ')

    const emailSent = await sendClaimConfirmationEmail({
      to: data.claimantEmail,
      claimantName: data.claimantName,
      documentTypeLabel,
      documentNumber: foundReport.documentNumber,
      station,
      foundLocation: foundReport.foundLocation,
    })

    await writeAuditLog({
      actorUserId: null,
      actorRole: null,
      action: 'REPORT_FOUND_CREATE',
      entityType: 'FOUND_REPORT',
      entityId: foundReport._id as ObjectId,
      message: 'Public claim submitted for found document',
      metadata: {
        claimId: claimResult.insertedId.toString(),
        foundReportId: data.foundReportId,
        emailSent,
      },
    })

    return NextResponse.json(
      {
        message: 'Claim submitted successfully',
        claimId: claimResult.insertedId.toString(),
        emailSent,
        station: {
          name: station.name,
          address: station.address,
          phone: station.phone,
        },
      },
      { status: 201 }
    )
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'Invalid input', details: error.errors },
        { status: 400 }
      )
    }
    console.error('Error creating claim:', error)
    return NextResponse.json({ error: 'Failed to submit claim' }, { status: 500 })
  }
}
